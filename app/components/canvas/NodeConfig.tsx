...
export function NodeConfig({
  ...
  const connectableComponents = component.connectsTo
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        <div className="p-3 bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg">
          <div className="flex items-start gap-2">
            <Info className="w-3.5 h-3.5 text-[#f38020] mt-0.5 flex-shrink-0" />
        ...
        {/* Connects to hints */}
        {connectableComponents.length > 0 && (
          <div>
            <p className="text-[11px] font-medium text-[#888] uppercase tracking-wider mb-2">
              Can connect to
            </p>
            <div className="flex flex-wrap gap-1.5">
              {connectableComponents.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="flex items-center gap-1.5 px-2 py-1 bg-[#1a1a1a] border border-[#222] rounded-md"
                    title={c.description}
                  >
                    <CIcon className="w-3 h-3 text-[#666]" />
